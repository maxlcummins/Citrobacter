...
write_delim(supp_table, "2022/delims/Supplementary_Table.txt", delim = "\t")
```
...
ggsave("2022/images/Figure2_Citro_abritamr_new.pdf", no_theme_abritamr, device = "pdf",  height = 8.27, width = 11.69)
...
#Save our theme
ggsave("2022/images/Figure2_Theme_horizontal_Citro_abritamr_new.pdf", abritamr_plot_theme_vertical, device = "pdf",  height = 8.27, width = 11.69)
```
...
ggsave("2022/images/Figure4_Virulence_genes.pdf", vfdb_heatmap2, device = "pdf",  height = 8.27, width = 11.69)
...
#Select our columns of interest
plasmidfinder <- plasmidfinder %>% select(EC_custom_intI1_HQ730118.1, EC_custom_intI2_MF150120.1, `ISfinder_Feb_2020_IS26:X00011`, starts_with("plasmidfinder")) 
...
#Create new names for our cell contents to help colorise things in our heatmap
plasmidfinder[plasmidfinder == 1] <- "Gene present"
plasmidfinder[plasmidfinder == 0] <- "Gene absent"
...
#Trim integron gene names
plasmidfinder_colnames_percs$long_name <- gsub("EC_custom_", "", plasmidfinder_colnames_percs$long_name)

plasmidfinder_colnames_percs$long_name <- gsub("_HQ730118.1", "", plasmidfinder_colnames_percs$long_name)

plasmidfinder_colnames_percs$long_name <- gsub("_MF150120.1", "", plasmidfinder_colnames_percs$long_name)

#Trim IS26 gene name
plasmidfinder_colnames_percs$long_name <- gsub("ISfinder_Feb_2020_", "", plasmidfinder_colnames_percs$long_name)

plasmidfinder_colnames_percs$long_name <- gsub(":X00011", "", plasmidfinder_colnames_percs$long_name)
...
plasmidfindergenotype_vars <- c("Gene present", "Gene absent")
...
plasmidfinder_heatmap2 <- plasmidfinder_heatmap  + scale_x_continuous(limits = c(-0.05, 0.25))

ggsave("2022/images/Figure5_plasmid_genes.pdf", plasmidfinder_heatmap2, device = "pdf",  height = 8.27, width = 11.69)

plasmidfinder_heatmap2
```
...
plas_df <- plasmidfinder %>% rownames_to_column('name')

combo_plas_res <- gene_by_sample %>% rownames_to_column('name') %>% full_join(plas_df, by = 'name')

combo_plas_res <- combo_plas_res %>% mutate(across(where(is.numeric), as.character)) %>% column_to_rownames('name')

#Plot our heatmap of card genes and plasmid genes
res_p3 <- gheatmap(
        p = p2,
        data = combo_plas_res,
        colnames_offset_y = 2,
        ...
        ylim(NA, nrow(gene_by_sample)*1.3) +
        xlim(c(-0.1,NA)) 
```
